...
export function detectThemeColors(css: string): string[] {
  ...
  const backgroundColorMatch = css.match(
    /--color-background:\s*(#[0-9a-fA-F]{3,8}|rgba?\([^)]+\))/i
  )
  ...
  const colorKeywords = {
    ...
    brown:
      /\b(brown|tan|beige|wood|wooden|walnut|oak|earth|dirt|soil|leather|coffee|chocolate|sepia|umber)\b/i,
